...
public class InstanceSetView extends RecordStorage {
  ...
  public void getRecords(RoutingContext routingContext) {
    try {
      ...
      var instancesObject = fakeStorageForImporting.instanceStorage.buildJsonRecordsResponse(query);
  ...
  private JsonObject getInstanceSet(JsonObject instance) {
    if (fakeStorageForImporting.precedingSucceedingStorage.failOnGetRecords) {
    ...
    fakeStorageForImporting.holdingsStorage.records.forEach((holdingId, holdingRecord) -> {
    ...
    fakeStorageForImporting.itemStorage.records.forEach((itemId, itemRecord) -> {
    ...
    fakeStorageForImporting.precedingSucceedingStorage.records.forEach((id, inventoryRecord) -> {
    ...
    fakeStorageForImporting.instanceRelationshipStorage.records.forEach((id, inventoryRecord) -> {
      var json = inventoryRecord.getJson();

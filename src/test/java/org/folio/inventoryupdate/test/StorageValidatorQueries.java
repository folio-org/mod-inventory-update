...
import static org.folio.inventoryupdate.test.InventoryUpdateTestSuite.INSTANCE_TYPE_ID_UNSPECIFIED;
...
public class StorageValidatorQueries
{
    ...
    protected void validateMatchKeyQuery (TestContext testContext) {
        FakeFolioApis.post(
                ...
                new InputInstance().setTitle("New Input Instance").setInstanceTypeId(INSTANCE_TYPE_ID_TEXT).setMatchKeyAsString("new_input_instance").setSource("test").getJson());

        FakeFolioApis.post(
                INSTANCE_STORAGE_PATH,
                new InputInstance().setTitle("Another Input Instance").setInstanceTypeId(INSTANCE_TYPE_ID_UNSPECIFIED).setMatchKeyAsString("another_input_instance").setSource("test").getJson());
    ...
    protected void validateOrQuery (TestContext testContext) {
        ...
        JsonObject responseOnOrQuery = FakeFolioApis.getRecordsByQuery(INSTANCE_STORAGE_PATH,
                "query="+RecordStorage.encode("matchKey==\"another_input_instance\" or instanceTypeId==\""+INSTANCE_TYPE_ID_UNSPECIFIED+"\""));
    ...
    protected void validateEqualityAndNotEqualityQuery (TestContext testContext) {
        JsonObject responseOnNotQuery1 = FakeFolioApis.getRecordsByQuery(INSTANCE_STORAGE_PATH,
                "query="+RecordStorage.encode("(identifiers =/@value/@identifierTypeId=\"4321\" \"888888\" not instanceTypeId==\""+INSTANCE_TYPE_ID_TEXT+"\")"));

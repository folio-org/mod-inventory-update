...
public class StorageValidatorHoldingsRecords {
    ...
    protected void validatePostAndGetById(TestContext testContext) {
        ...
        JsonObject responseOnGET = FakeFolioApisForImporting.getRecordById(FakeFolioApisForImporting.HOLDINGS_STORAGE_PATH, responseOnPOST.getString("id"));
    ...
    protected void validateGetByQueryAndPut(TestContext testContext) {
        ...
        testContext.assertEquals(
                responseJson.getInteger("totalRecords"), 1,"Number of " + FakeFolioApisForImporting.RESULT_SET_HOLDINGS_RECORDS + " expected: 1" );
        JsonObject existingRecord = responseJson.getJsonArray(FakeFolioApisForImporting.RESULT_SET_HOLDINGS_RECORDS).getJsonObject(0);
        existingRecord.put("callNumber", SECOND_CALL_NUMBER);
        FakeFolioApisForImporting.put(FakeFolioApisForImporting.HOLDINGS_STORAGE_PATH, existingRecord);
        JsonObject holdingsRecord = FakeFolioApisForImporting.getRecordById(FakeFolioApisForImporting.HOLDINGS_STORAGE_PATH, existingRecord.getString("id"));
    ...
    protected void validateCanDeleteHoldingsRecordById (TestContext testContext) {
        ...
        FakeFolioApisForImporting.delete(FakeFolioApisForImporting.HOLDINGS_STORAGE_PATH, responseOnPOST.getString("id"),200);
    }
    ...
    protected void validateCannotPostWithBadInstanceId (TestContext testContext) {
        ...
        testContext.assertEquals(
                responseJson.getInteger("totalRecords"), 0,"Number of " + FakeFolioApisForImporting.RESULT_SET_HOLDINGS_RECORDS + " expected for bad instance ID " + NON_EXISTING_INSTANCE_ID + ": 0" );
    ...
    protected void validateCannotPostWithBadLocationId (TestContext testContext) {
        ...
        testContext.assertEquals(
                responseJson.getInteger("totalRecords"), 0,"Number of " + FakeFolioApisForImporting.RESULT_SET_HOLDINGS_RECORDS + " expected for bad location ID " + NON_EXISTING_INSTANCE_ID + ": 0" );
